"""
copied from https://github.com/lucidrains/perceiver-pytorch
@wx
"""
...
class Attention(nn.Module):
    ...
    def forward(self, x, context = None, mask = None):
        ...
        if (not USE_XOPS) or exists(mask):
            ...
            attn = xops.memory_efficient_attention(q,k,v, p=self.dropout.p)
            out = rearrange(attn, 'b n h d -> b n (h d)', h = h)
...
class Perceiver(nn.Module):
    ...
    def forward(
        ...
        assert len(axis) == self.input_axis, 'input data must have the right number of axis' # eq. seqlen in the transformer
...
def get_perceiver(cfg):
    return Perceiver(
                input_channels = cfg["input_chan"],
...
if __name__=="__main__":
    ...
    tt = len(list(model.named_parameters()))
    print(tt)