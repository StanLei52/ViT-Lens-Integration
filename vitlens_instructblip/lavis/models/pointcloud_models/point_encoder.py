...
class PointTransformer(nn.Module):
    def __init__(self, config, output_dim=None):
        ...
        if output_dim is not None:
            self.proj = nn.Parameter(torch.empty(cat_factor*self.trans_dim, output_dim))
    ...
    def forward(self, pts):
        ...
        x = self.blocks(x, pos)
        x = self.norm(x)
        concat_f = torch.cat([x[:, 0], x[:, 1:].max(1)[0]], dim=-1) if self.do_cat else x[:, 0]
...
class PointTokenizer(nn.Module):
    def __init__(self, trans_dim, group_size, num_group, encoder_dim):
        super().__init__()
